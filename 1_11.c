...
#include <math.h>
  ...
  int main ()
    ...
    int nCoeff = 0;
    double prover_coeff[3] = {0};

    for (; nCoeff < 3; nCoeff++)
      {
      while (scanf("%lf", &prover_coeff[nCoeff]) != 1)
        {
    ...
    double a = prover_coeff[0];
    double b = prover_coeff[1];
    double c = prover_coeff[2];
    ...
    switch (solution)
      ...
      default: printf ("ERROR!!! Invalid data entry!!!");
               return 1;
      }
    }
